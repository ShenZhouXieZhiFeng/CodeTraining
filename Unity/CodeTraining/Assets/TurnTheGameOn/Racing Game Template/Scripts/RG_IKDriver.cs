...
public class RG_IKDriver : MonoBehaviour {
	...
	public float shiftAnimSpeed = 20.0f;
	...
	void Start () {
		...
		if (RGT_PlayerPrefs.inputData.useMobileController) mobile = true;
		...
		if (animator) {			
			if (ikActive) {
				...
				if (rightHandObj != null) {
					...
					if (shifting) {
						targetRightHandIK.position = Vector3.Lerp (targetRightHandIK.position, shiftObj.position, shiftAnimSpeed * Time.deltaTime);
						targetRightHandIK.rotation = Quaternion.Lerp (targetRightHandIK.rotation, shiftObj.rotation, shiftAnimSpeed * Time.deltaTime);
					} else {
