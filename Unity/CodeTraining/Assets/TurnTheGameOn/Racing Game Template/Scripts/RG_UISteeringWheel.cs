...
namespace TurnTheGameOn.RacingGameTemplate{
	public class RG_UISteeringWheel : MonoBehaviour {
		...
		void Start(){
			...
			if (!RG_CrossPlatformInputManager.AxisExists(axisName)){
				...
				m_HorizontalVirtualAxis = new RG_CrossPlatformInputManager.VirtualAxis(axisName);
				RG_CrossPlatformInputManager.RegisterVirtualAxis(m_HorizontalVirtualAxis);
			} else {
				m_HorizontalVirtualAxis = RG_CrossPlatformInputManager.VirtualAxisReference(axisName);
			}
		}
				...
				if (RGT_PlayerPrefs.inputData.useMobileController) {
					if( Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Moved){
						for(int i = 0; i < Input.touchCount; i++){
							...
							touchPosition.y = Screen.height - Input.touches[i].position.y;
